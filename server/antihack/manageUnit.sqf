...
if (!isDedicated && isNil _checksum) then
{
	...
	if (!isServer) then
	{
		...
		TPG_fnc_MP = compileFinal (_assignPacketKey + (preprocessFileLineNumbers "server\functions\remote\fn_MP.sqf"));
		call compile (_assignPacketKey + (preprocessFileLineNumbers "server\functions\remote\fn_initMultiplayer.sqf"));
	};
