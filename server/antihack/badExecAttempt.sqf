...
if (typeName _this == typeName [] && {count _this > 0} && {typeName (_this select 0) == typeName ""} && {_this select 0 == "BIS_fnc_MP_packet"}) then
{
	...
	if ((toLower (_values select 1)) in ["bis_fnc_kbtelllocal", "bis_fnc_effectkilledairdestruction", "bis_fnc_effectkilledsecondaries", "bis_fnc_effectkilledairdestructionstage2", "bis_fnc_tridenthandledamage", "bis_fnc_tridenthandledamage_server", "bis_fnc_shownotification", "bis_fnc_tasksetstate"] || {_defineServerRules}) then
	{
